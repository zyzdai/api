...
def getKey(baseUrl):
    ...
    url = xGorgon("https://reading.snssdk.com/reading/crypt/registerkey?" + getLoginHeaders(),gzip_data(data_json))
    uri = str(url).split(",{")
    url = uri[0]
    ...
    obj = requests.post(url, headers=headers, data=decoded_bytes).json()['data']
...
def xGorgon(url, data=None):
    ...
    def calculate(hex_str):
        ...
        T = int(time.time())
        param_list.extend([(T >> 24) & 0xFF, (T >> 16) & 0xFF, (T >> 8) & 0xFF, T & 0xFF])

        eor_result_list = [param ^ key[i % length] for i, param in enumerate(param_list)]
...
def getContent(item_id):
    baseUrl = "https://reading.snssdk.com/reading/reader/full/v/?item_id="+item_id+getLoginHeaders()
